...
class TestCheckStructure:
    def test_deterministic_methods_still_on_collector(self):
        ...
        deterministic_methods = [
            "contains", "not_contains", "max_tokens", "min_tokens",
            "latency", "cost", "json_schema", "regex", "exact_match",
            "starts_with", "ends_with", "word_count", "char_count",
            "similarity", "readability", "sentence_count", "all_of",
            "any_of", "none_of", "is_json", "is_valid_python", "no_pii",
            "language", "greater_than", "less_than", "between",
            "bleu", "rouge_l", "json_field", "is_valid_sql",
            "is_valid_markdown",
        ]
    ...
    def test_infrastructure_methods_on_collector(self):
        config = CheckllmConfig()
        c = CheckCollector(config=config)
        infra = ["that", "expect", "teardown", "total_cost", "cache_stats",
                 "run_metric", "aflush", "_get_judge", "_cached_judge_check"]
    ...
    def test_async_judge_methods_on_collector(self):
        ...
        c = CheckCollector(config=config)
        async_methods = [
            "ahallucination", "arelevance", "atoxicity", "arubric",
            "afluency", "acoherence", "asentiment", "acorrectness",
        ]
