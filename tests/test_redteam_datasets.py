...
class TestEachPreset:
    def test_returns_non_empty_list(self, preset):
        ...
        assert len(prompts) >= 10, (
            f"preset {preset!r} should ship at least 10 curated prompts"
        )
    ...
    def test_prompts_are_reasonably_sized(self, preset):
        for item in load_jailbreak_preset(preset):
            assert len(item.prompt) < 4000, (
                "curated prompts should stay compact"
            )
        ...
        for item in load_jailbreak_preset(preset):
            ...
            for term in _FORBIDDEN_TERMS:
                assert term not in lowered, (
                    f"preset {preset!r} contains forbidden term {term!r}"
                )

    def test_has_source_metadata(self, preset):
        for item in load_jailbreak_preset(preset):
            assert item.source, (
                f"preset {preset!r} missing source metadata"
            )
            assert item.citation, (
                f"preset {preset!r} missing citation metadata"
            )

    def test_prompts_are_unique_within_preset(self, preset):
        prompts = [p.prompt for p in load_jailbreak_preset(preset)]
        assert len(set(prompts)) == len(prompts), (
            f"preset {preset!r} contains duplicate prompts"
        )
