...
def _scalar_macros(
    ...
    latency_ratio = _get(s4, "deepeval", "mean_latency_ms_per_trajectory") / _get(
        s4, "checkllm", "mean_latency_ms_per_trajectory"
    )
    out.append(
        _command(
...
def _table_head_to_head(s4: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        "Coverage & " f"{_round3(cl['coverage'])} & {_round3(de['coverage'])}{LINE_END}",
        "Cost (USD / traj.) & "
